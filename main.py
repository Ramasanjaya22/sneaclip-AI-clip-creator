...
flask_secret = __os.environ.get('FLASK_SECRET_KEY')
if is_prod and not flask_secret:
    raise ValueError("FLASK_SECRET_KEY must be set in production.")
app.secret_key = flask_secret or "dev_default_secret_key"
...
def set_cache_headers(response):
    ...
    response.headers["Content-Security-Policy"] = "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.plyr.io https://unpkg.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdn.plyr.io; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: blob:; media-src 'self' blob:; connect-src 'self'"
            ...
            try:
                ...
                if video and video.filename != '':
                    if not allowed_file_extension(video.filename, ALLOWED_EXTENSIONS_VIDEO):
                        return jsonify({"success": False, "error": "Invalid video file type"}), 400
...
def upload_watermark():
    try:
        ...
        if not allowed_file_extension(file.filename, ALLOWED_EXTENSIONS_IMAGE):
            return jsonify({"success": False, "error": "Invalid watermark file type"}), 400
        filename = secure_filename(file.filename)
...
def upload_chunk():
    upload_id_raw = request.headers.get("X-Upload-Id")
    chunk_index = request.headers.get("X-Chunk-Index")
    
    upload_id = secure_filename(upload_id_raw) if upload_id_raw else None
...
def finalize_upload():
    data = request.get_json(force=True)
    upload_id_raw = data.get("upload_id")
    upload_id = secure_filename(upload_id_raw) if upload_id_raw else None
...
def process_video_async():
    data = request.get_json(force=True)
    job_id_raw = data.get("job_id") or data.get("upload_id")
    job_id = secure_filename(job_id_raw) if job_id_raw else None
