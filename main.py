...
# Secret Key
if is_prod and not __os.environ.get('FLASK_SECRET_KEY'):
    raise ValueError("FLASK_SECRET_KEY environment variable is required in production.")
app.secret_key = __os.environ.get('FLASK_SECRET_KEY', 'dev-secret-key-default-123')
...
ALLOWED_EXTENSIONS_VIDEO = {'mp4', 'mkv', 'avi', 'mov', 'flv', 'webm', 'm4v'}
...
def set_cache_headers(response):
    ...
    response.headers["Content-Security-Policy"] = "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdn.plyr.io; font-src 'self' https://fonts.gstatic.com; img-src 'self' data:; media-src 'self' blob:; connect-src 'self';"
