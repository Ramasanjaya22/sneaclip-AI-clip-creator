...
# Setup secret key for secure sessions
app.secret_key = __os.environ.get('FLASK_SECRET_KEY')
if not app.secret_key:
    if is_prod:
        raise ValueError("No FLASK_SECRET_KEY set for Flask application in production environment.")
    app.secret_key = "dev-fallback-secret-key-do-not-use-in-prod"
...
def set_cache_headers(response):
    ...
    response.headers["Content-Security-Policy"] = "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.plyr.io https://unpkg.com; style-src 'self' 'unsafe-inline' https://cdn.plyr.io https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: blob:; media-src 'self' blob:; connect-src 'self'"
...
def upload_chunk():
    ...
    if upload_id: upload_id = secure_filename(str(upload_id))
...
def process_video_async():
    ...
    if job_id: job_id = secure_filename(str(job_id))
