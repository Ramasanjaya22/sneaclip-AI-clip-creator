...
def set_cache_headers(response):
    ...
    response.headers["Content-Security-Policy"] = "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.plyr.io https://unpkg.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdn.plyr.io; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: blob: https://via.placeholder.com; media-src 'self' blob:; connect-src 'self' blob:; frame-src 'self';"
            ...
            try:
                ...
                if video:
                    ...
                    if not allowed_file_extension(video.filename, ALLOWED_EXTENSIONS_VIDEO):
                        return render_template("index.html", config=config, error="Invalid video file format.", folders=get_files(clip_folder))
...
def upload_music():
    try:
        ...
        if not allowed_file_extension(file.filename, ALLOWED_EXTENSIONS_MUSIC):
            return jsonify({"success": False, "error": "Invalid music file format."}), 400
...
def upload_watermark():
    try:
        ...
        if not allowed_file_extension(file.filename, ALLOWED_EXTENSIONS_IMAGE):
            return jsonify({"success": False, "error": "Invalid watermark image format."}), 400
