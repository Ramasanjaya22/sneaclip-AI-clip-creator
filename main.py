...
def is_valid_id(identifier):
    """Validates that the identifier only contains alphanumeric characters, hyphens, and underscores."""
    if not identifier:
        return False
...
app.secret_key = __os.environ.get('FLASK_SECRET_KEY', 'default-dev-key-fallback')
...
def upload_watermark():
    try:
        ...
        if not file or file.filename == "":
            return jsonify({"success": False, "error": "Empty file"}), 400
        if not allowed_file_extension(file.filename, {'.png', '.jpg', '.jpeg', '.webp'}):
