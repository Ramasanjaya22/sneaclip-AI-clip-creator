...
def set_cache_headers(response):
    ...
    response.headers["Content-Security-Policy"] = "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.plyr.io https://unpkg.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdn.plyr.io; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: blob:;"
            ...
            try:
                ...
                if video:
                    if not allowed_file_extension(video.filename, ALLOWED_EXTENSIONS_VIDEO):
                        return render_template("index.html", config=config, error="Invalid video format", folders=get_files(clip_folder))
...
def finalize_upload():
    ...
    try:
        from models.chunked_upload import finalize_upload as do_finalize_upload
        result = do_finalize_upload(upload_id)
        return jsonify(result)
    except ValueError as e:
        return jsonify({"error": safe_error(e)}), 400
