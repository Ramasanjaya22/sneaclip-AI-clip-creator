...
def export_status(job_id):
    if not re.match(r'^[\w-]+$', str(job_id)):
...
def upload_chunk():
    ...
    # Validation uses regex, not secure_filename
    if not re.match(r'^[\w-]+$', str(upload_id)):
