...
def set_cache_headers(response):
    ...
    response.headers["Content-Security-Policy"] = "default-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:;"
            ...
            try:
                ...
                if video:
                    if not allowed_file_extension(video.filename, ALLOWED_EXTENSIONS_VIDEO):
                        return render_template("index.html", config=config, error="Invalid video file extension", folders=get_files(clip_folder))
...
def upload_watermark():
    try:
        ...
        if not allowed_file_extension(file.filename, ALLOWED_EXTENSIONS_IMAGE):
            return jsonify({"success": False, "error": "Invalid watermark image file extension"}), 400
        filename = secure_filename(file.filename)
