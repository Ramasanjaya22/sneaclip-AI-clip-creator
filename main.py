...
def set_cache_headers(response):
    ...
    response.headers["Content-Security-Policy"] = "default-src 'self' 'unsafe-inline' 'unsafe-eval' data: https://cdn.plyr.io https://unpkg.com https://fonts.googleapis.com https://fonts.gstatic.com blob:; media-src 'self' blob:;"
                ...
                if video and video.filename:
                    if not allowed_file_extension(video.filename, ALLOWED_EXTENSIONS_VIDEO):
                        return render_template(
                            "index.html",
                            config=config,
                            error="Invalid file type. Allowed: " + ", ".join(ALLOWED_EXTENSIONS_VIDEO),
...
def upload_watermark():
    try:
        ...
        if not allowed_file_extension(file.filename, ALLOWED_EXTENSIONS_IMAGE):
            return jsonify({"success": False, "error": "Invalid watermark image type"}), 400
        filename = secure_filename(file.filename)
