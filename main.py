...
secret_key = __os.environ.get('FLASK_SECRET_KEY')
if is_prod and not secret_key:
    raise ValueError("No FLASK_SECRET_KEY set for production application")
app.secret_key = secret_key or "dev-secret-key-fallback"
...
def set_cache_headers(response):
    ...
    response.headers["Content-Security-Policy"] = "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.plyr.io https://unpkg.com; style-src 'self' 'unsafe-inline' https://cdn.plyr.io https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: blob:;"
            ...
            try:
                ...
                if video:
                    if not allowed_file_extension(video.filename, ALLOWED_EXTENSIONS_VIDEO):
                        return render_template("index.html", config=config, error="Invalid video file type", folders=get_files(clip_folder))
...
def upload_music():
    try:
        ...
        if not allowed_file_extension(file.filename, ALLOWED_EXTENSIONS_MUSIC):
            return jsonify({"success": False, "error": "Invalid music file type"}), 400
...
def upload_watermark():
    try:
        ...
        if not allowed_file_extension(file.filename, ALLOWED_EXTENSIONS_IMAGE):
            return jsonify({"success": False, "error": "Invalid image file type"}), 400
        filename = secure_filename(file.filename)
...
def upload_chunk():
    upload_id = secure_filename(str(request.headers.get("X-Upload-Id", ""))) if request.headers.get("X-Upload-Id") else None
...
def finalize_upload():
    data = request.get_json(force=True)
    upload_id = secure_filename(str(data.get('upload_id'))) if data.get('upload_id') else None
...
def process_video_async():
    data = request.get_json(force=True)
    raw_job_id = data.get('job_id') or data.get('upload_id')
    job_id = secure_filename(str(raw_job_id)) if raw_job_id else None
