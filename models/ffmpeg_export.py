...
def _build_filter_complex(video_path, clips, editor_options):
    ...
    if len(clips) > 1:
        # Interleave the streams to be [0:v:0][0:a:0][1:v:0][1:a:0]
        concat_str = ""
        for idx in range(len(clips)):
            concat_str += f"[{idx}:v:0]"
            if has_audio:
                concat_str += f"[{idx}:a:0]"
        if has_audio:
    ...
    if aspect == "9:16":
        ...
        if blur_bg:
            ...
            filters.append(
                ...
                f"[bg_full]scale={bg_target_w}:{bg_target_h}:flags=fast_bilinear:force_original_aspect_ratio=increase,crop={bg_target_w}:{bg_target_h},boxblur=luma_radius=min(h\\,w)/18:luma_power=1,scale={target_w}:{target_h}[bg];"
    ...
    if music_path and os.path.exists(music_path):
        ...
        # Pass through audio unmodified, but use anull to put it in the filter graph
