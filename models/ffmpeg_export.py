...
def _build_filter_complex(video_path, clips, editor_options):
    ...
    if music_path and os.path.exists(music_path):
        ...
        # Pass-through orig audio so we can assign it a known label to avoid -map sequence errors
...
def _has_audio_stream(video_path):
    try:
        import subprocess
        r = subprocess.run(
            [get_ffprobe_exe(), "-v", "error", "-select_streams", "a", "-show_entries", "stream=codec_type", "-of", "csv=p=0", video_path],
