...
def _has_audio_stream(video_path):
    try:
        ...
        r = subprocess.run(
            [get_ffprobe_exe(), "-hide_banner", "-v", "error", "-select_streams", "a", "-show_entries", "stream=codec_type", "-of", "default=noprint_wrappers=1:nokey=1", video_path],
            capture_output=True, text=True, creationflags=subprocess.CREATE_NO_WINDOW if os.name == "nt" else 0
        )
        return "audio" in r.stdout.strip().lower()
    except Exception:
        return True
